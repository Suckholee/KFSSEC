...
export default function CalendarDateCoursesModal({
  ...
  const handleEnrollSubmit = (e) => {
    ...
    if (!enrollForm.name || !enrollForm.phone) {
      alert(tr('성함과 연락처를 입력해 주세요.'));
      return;
    }

    setEnrollSuccess(true);
    setTimeout(() => {
      alert(tr`🎉 [${enrollingCourse.title}] 수강 신청 접수가 완료되었습니다.\n담담 연구원이 24시간 이내 연락처(${enrollForm.phone})로 안내 문자를 발송합니다.`);
          ...
          {enrollingCourse ? (
            <div className="bg-emerald-50/70 p-6 sm:p-7 rounded-3xl border-2 border-[#0B3C26] space-y-5 animate-fadeIn">
              <div className="flex items-center justify-between border-b border-emerald-200 pb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-600 font-bold mt-0.5">
                    {tr(enrollingCourse.instructor || '명장 전수 강좌')}{tr(" · 수강료 ")}{tr(enrollingCourse.priceFormatted || tr`${enrollingCourse.price?.toLocaleString()}원`)}
                  </p>
                </div>
                <button
              ...
              {enrollSuccess ? (
                <div className="py-10 text-center space-y-3">
                  ...
                  <p className="text-xs text-gray-600 font-bold">{tr("잠시만 기다려주시면 접수가 완료됩니다.")}</p>
                </div>
              ) : (
                <form onSubmit={handleEnrollSubmit} className="space-y-4 text-xs font-bold">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-gray-700 font-bold mb-1">{tr("신청자 성함 *")}</label>
                      <input
                        type="text"
                        required
                        placeholder={tr("홍길동")}
                    ...
                    <div>
                      <label className="block text-gray-700 font-bold mb-1">{tr("연락처 (휴대폰) *")}</label>
                      <input
                        type="tel"
                        required
                  ...
                  <div>
                    <label className="block text-gray-700 font-bold mb-1">{tr("창업 상태 및 관심 분야")}</label>
                    <select
                  ...
                  <div>
                    <label className="block text-gray-700 font-bold mb-1">{tr("상담 희망사항 / 메모 (선택)")}</label>
                    <textarea
                      rows={2}
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500 font-bold">{tr("선착순 15명 정원 마감 임박")}</span>
              </div>
                    ...
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-black shrink-0 relative border border-gray-200">
                        ...
                        <span className="absolute top-1 left-1 px-1.5 py-0.5 rounded bg-black/80 text-white text-[10px] font-bold">
                          {tr(c.categoryName || c.industry || '인기과정')}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1.5 flex-1">
                        ...
                        <p className="text-xs text-gray-600 font-medium line-clamp-1">
                          {tr(c.description || '외식창업 실무 레시피 및 매장 운영 전수')}
                        </p>
