    ...
    <aside className="w-full lg:w-64 shrink-0 space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-emerald-100 overflow-hidden shadow-sm">
        ...
        <div className="bg-[#0F5132] px-6 py-7 text-center">
          <h2 className="text-xl font-extrabold text-white tracking-tight">{tr(" 교육과정 ")}</h2>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={isActive ? 'ml-1' : ''}>{tr(item.label)}</span>
              </button>
            );
          })}
        </nav>
      ...
      <div className="bg-white rounded-2xl border border-emerald-100 p-5 shadow-sm space-y-4">
        <div className="flex items-center gap-2 text-gray-800">
          ...
          <span className="text-sm font-extrabold">{tr("교육 상담센터")}</span>
        </div>
        ...
        <div>
          ...
          <span className="text-xs text-gray-400 font-medium">{tr(" 평일 09:00 ~ 18:00 ")}</span>
        </div>

        <div className="space-y-2 pt-1">
          <button
            onClick={() => alert(tr('1:1 문의하기 페이지로 이동합니다.'))}
          ...
          >
            ...
            <span>{tr("1:1 문의하기")}</span>
          </button>
          <button
            onClick={() => alert(tr('자주 묻는 질문 FAQ로 이동합니다.'))}
            className="w-full py-2.5 px-3 border border-emerald-200 rounded-xl text-xs font-bold text-emerald-900 hover:bg-emerald-50 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
          >
            <HelpCircle className="w-3.5 h-3.5 text-emerald-600" />
            <span>{tr("자주 묻는 질문")}</span>
          </button>
        </div>
      </div>
    </aside>
