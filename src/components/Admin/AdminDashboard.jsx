...
import { Users, GraduationCap, TrendingUp, CreditCard, ChevronRight, ArrowUpRight, ArrowDownRight, Clock, CheckCircle2, AlertCircle } from 'lucide-react';
    ...
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="bg-gradient-to-r from-emerald-950 via-[#0a2318] to-[#0d1c15] p-6 rounded-3xl border border-emerald-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
        ...
        <button
          ...
          className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs sm:text-sm rounded-xl shadow-lg transition-all shrink-0 cursor-pointer flex items-center gap-1.5"
        >
          <span>수강 신청 전체 관리</span>
            ...
            <tbody className="divide-y divide-emerald-900/30 text-xs sm:text-sm font-semibold text-gray-200">
              {recentApplications.map((app) => (
                <tr key={app.id} className="hover:bg-emerald-950/40 transition-colors">
                  <td className="py-3.5 px-2 font-mono text-xs text-gray-400">{app.id}</td>
                  <td className="py-3.5 px-2 font-black text-white">{app.name}</td>
