...
export default function CourseEditModal({ isOpen, course, onClose, onSaveCourse }) {
  ...
  useEffect(() => {
    if (course) {
      setFormData({
        ...
        originalPrice: course.originalPrice || course.price || '1,500,000원',
  ...
  const priceCalc = calculateDiscount(formData.originalPrice, formData.discount);
  ...
  const handleSubmit = (e) => {
    ...
    // Save both original price and calculated discounted price
    const finalCourse = {
      ...formData,
      price: priceCalc.discountedPriceStr,
      originalPrice: priceCalc.originalPriceStr,
    };
      ...
      <div className="relative w-full max-w-2xl bg-[#0d1712] rounded-3xl border border-emerald-500/40 p-6 sm:p-8 shadow-2xl space-y-6 text-white max-h-[90vh] overflow-y-auto no-scrollbar">
        ...
        <div className="border-b border-emerald-900/60 pb-4 space-y-1">
          <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs font-black">
            AUTOMATED PRICE & DISCOUNT CALCULATOR
          </span>
          <h2 className="text-2xl font-black text-white">
            {course ? '교육과정 정가 & 할인율(%) 실시간 계산 수정' : '신규 교육과정 등록'}
          </h2>
          <p className="text-xs text-emerald-300/80 font-medium">
            정가와 할인율(%)을 입력하면 할인 반영 최종 수강료가 자동 계산되어 정가(취소선)와 함께 명확히 표시됩니다.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs sm:text-sm">
            ...
            <div className="space-y-1">
              <label className="font-extrabold text-emerald-300">원래 가격 (정가)</label>
              <input
                type="text"
            ...
            {/* Discount Percentage Badge Input */}
            <div className="space-y-1">
              <label className="font-extrabold text-emerald-300">할인율 (%) 및 뱃지 텍스트</label>
              <input
                type="text"
                value={formData.discount}
                onChange={(e) => setFormData({ ...formData, discount: e.target.value })}
                placeholder="예: 20% OFF, 15% OFF"
                className="w-full bg-[#16241c] border border-emerald-500/30 rounded-xl px-3.5 py-2.5 text-white font-bold focus:outline-none focus:border-emerald-400"
              />
            </div>

            {/* Real-time Calculated Final Price Box */}
            <div className="sm:col-span-2 bg-[#111c16] p-4 rounded-2xl border border-emerald-500/30 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Calculator className="w-5 h-5 text-emerald-400" />
                <div>
                  <span className="text-xs text-gray-400 font-bold block">할인율 자동 적용 최종 수강료</span>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-emerald-300 text-lg sm:text-xl font-black">
              ...
              {priceCalc.hasDiscount && (
                <span className="px-3 py-1 bg-rose-500 text-white rounded-lg text-xs font-black shadow-md">
                  {priceCalc.ratePercent}% 할인 혜택 적용됨
                </span>
              )}
            </div>
