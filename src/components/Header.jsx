...
export default function Header({
  ...
  return (
    <header className="relative bg-[#FDFBF7] border-b-2 border-[#E7E2D8] z-50 transition-all font-sans text-gray-900 shadow-xs">
      ...
      <div className="w-full px-6 sm:px-10 lg:px-14 h-20 sm:h-24 flex items-center justify-between gap-6">
              ...
              <div key={menu.key} className="relative group py-6 cursor-pointer">
                <button
                  ...
                  className={`text-base lg:text-lg font-black tracking-tight transition-all flex items-center gap-1 cursor-pointer whitespace-nowrap ${
                    isMenuActive
                      ? 'text-[#0B3C26] border-b-2 border-[#C5A059] pb-0.5 scale-105'
        ...
        <div className="hidden md:flex items-center gap-5 shrink-0">
          ...
          <div className="relative">
            <button
              ...
              className="px-4 py-1.5 bg-[#F2ECE0] border border-[#D4C5B0] text-[#0B3C26] text-xs font-black rounded-full flex items-center gap-2 cursor-pointer shadow-xs hover:bg-[#EBE2D4] transition-colors"
            >
              <span>{lang}</span>
          ...
          {currentUser ? (
            <div className="flex items-center gap-3 border-l border-[#E5E0D8] pl-5">
              <div className="flex items-center gap-2">
                <div className="bg-[#0B3C26] p-1.5 rounded-full text-white">
                  <User className="w-3.5 h-3.5" />
                </div>
                <span className="text-xs font-black text-[#0B3C26]">
                  {currentUser.name || '수강생 회원'}님
                </span>
              </div>
              <button
                onClick={onLogout}
                className="px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-800 text-xs font-black rounded-xl border border-rose-300 transition-colors cursor-pointer flex items-center gap-1"
              >
            ...
            <button
              ...
              className="group px-5 py-2.5 bg-[#0B3C26] hover:bg-[#072819] text-white rounded-2xl shadow-md transition-all cursor-pointer border border-[#0B3C26] flex items-center gap-2"
            >
        ...
        <button
          ...
          className="md:hidden p-2 text-[#0B3C26] hover:text-black rounded-xl"
        >
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#F8F6F0] border-b border-[#E7E2D8] p-6 space-y-6 animate-fadeIn">
          <div className="flex items-center justify-end pb-4 border-b border-[#E7E2D8]">
            {currentUser ? (
              <div className="flex items-center gap-3">
                <span className="text-xs font-black text-[#0B3C26]">{currentUser.name}님</span>
                <button
                  onClick={() => {
                    onLogout();
                    setMobileMenuOpen(false);
                  }}
                  className="px-3 py-1.5 bg-rose-600 text-white text-xs font-black rounded-xl"
                >
                  로그아웃
                </button>
              </div>
              ...
              <button
                ...
                className="px-4 py-2 bg-[#0B3C26] text-white text-xs font-black rounded-xl"
              >
                # LOGIN / JOIN US
              </button>
            )}
          ...
          <div className="space-y-4">
            {mainMenuItems.map((menu) => (
              <button
                ...
                className={`w-full text-left py-2 text-base font-black border-b border-[#E7E2D8] ${
                  activeTab === menu.key ? 'text-[#0B3C26] font-black' : 'text-gray-800 hover:text-[#C5A059]'
                }`}
              >
                {menu.title}
              </button>
